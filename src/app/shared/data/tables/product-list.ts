export class productDB {
    static product = [
        {
            imagen: "<img src='assets/images/ecommerce/product-table-1.png'>",
            nombre: "Red Lipstick",
            descripcion: "Interchargebla lens Digital Camera with APS-C-X Trans CMOS Sens",
            precio: "$10",
            stock: "<div class='font-success'>In Stock</div>",
            fecha: "2011/4/19"
        },
        {
            imagen: "<img src='assets/images/ecommerce/product-table-2.png'>",
            nombre: "Red Lipstick",
            descripcion: "Interchargebla lens Digital Camera with APS-C-X Trans CMOS Sens",
            precio: "$10",
            stock: "<div class='font-danger'>Out of Stock</div>",
            fecha: "2011/4/19"
        },
        {
            imagen: "<img src='assets/images/ecommerce/product-table-3.png'>",
            nombre: "Red Lipstick",
            descripcion: "Interchargebla lens Digital Camera with APS-C-X Trans CMOS Sens",
            precio: "$10",
            stock: "<div class='font-danger'>Out of Stock</div>",
            fecha: "2011/4/19"
        },
        {
            imagen: "<img src='assets/images/ecommerce/product-table-4.png'>",
            nombre: "Red Lipstick",
            descripcion: "Interchargebla lens Digital Camera with APS-C-X Trans CMOS Sens",
            precio: "$20",
            stock: "<div class='font-primary'>Low Stock</div>",
            fecha: "2011/4/19"
        },
        {
            imagen: "<img src='assets/images/ecommerce/product-table-5.png'>",
            nombre: "Red Lipstick",
            descripcion: "Interchargebla lens Digital Camera with APS-C-X Trans CMOS Sens",
            precio: "$30",
            stock: "<div class='font-success'>In Stock</div>",
            fecha: "2011/4/19"
        },
        {
            imagen: "<img src='assets/images/ecommerce/product-table-6.png'>",
            nombre: "Brown Lipstick",
            descripcion: "Interchargebla lens Digital Camera with APS-C-X Trans CMOS Sens",
            precio: "$40",
            stock: "<div class='font-success'>In Stock</div>",
            fecha: "2011/4/19"
        },
        {
            imagen: "<img src='assets/images/ecommerce/product-table-1.png'>",
            nombre: "Red Lipstick",
            descripcion: "Interchargebla lens Digital Camera with APS-C-X Trans CMOS Sens",
            precio: "$10",
            stock: "<div class='font-success'>In Stock</div>",
            fecha: "2011/4/19"
        },
        {
            imagen: "<img src='assets/images/ecommerce/product-table-2.png'>",
            nombre: "Red Lipstick",
            descripcion: "Interchargebla lens Digital Camera with APS-C-X Trans CMOS Sens",
            precio: "$10",
            stock: "<div class='font-success'>In Stock</div>",
            fecha: "2011/4/19"
        }
    ];
    static orderHistory = [
        {
            img: "<img class='img-fluid img-60' src='assets/images/product/1.png'>",
            name: 'Long Top',
            size: 'M',
            color: 'Lavander',
            articleNo: '4215738',
            unit: '11',
            price: '$21'
        },
        {
            img: "<img class='img-fluid img-60' src='assets/images/product/13.png'>",
            name: 'Fancy Watch',
            size: '35 mm',
            color: 'Blue',
            articleNo: '5476182',
            unit: '11',
            price: '$10'
        },
        {
            img: "<img class='img-fluid img-60' src='assets/images/product/4.png'>",
            name: 'Man Shoes',
            size: '8',
            color: 'Black & White',
            articleNo: '1756457',
            unit: '11',
            price: '$ 18'
        },
        {
            img: "<img class='img-fluid img-60' src='assets/images/product/10.png'>",
            name: 'Ledis side bag',
            size: '22cm x 18cm',
            color: 'brown',
            articleNo: '7451725',
            unit: '1',
            price: '$13'
        },
        {
            img: "<img class='img-fluid img-60' src='assets/images/product/12.png'>",
            name: 'ledis slipper',
            size: '6',
            color: 'brown',
            articleNo: '4127421',
            unit: '1',
            price: '$6'
        },
        {
            img: "<img class='img-fluid img-60' src='assets/images/product/3.png'>",
            name: 'Fancy ledis Jacket',
            size: 'Xl',
            color: 'Light Gray',
            articleNo: '3581714',
            unit: '1',
            price: '$24'
        },
        {
            img: "<img class='img-fluid img-60' src='assets/images/product/2.png'>",
            name: 'Ledis Handbag',
            size: '25x25',
            color: 'Black',
            articleNo: '6748142',
            unit: '1',
            price: '$12'
        },
        {
            img: "<img class='img-fluid img-60' src='assets/images/product/15.png'>",
            name: 'Iphone 6',
            size: '15x15',
            color: 'gold',
            articleNo: '5748214',
            unit: '1',
            price: '$25'
        },
        {
            img: "<img class='img-fluid img-60' src='assets/images/product/14.png'>",
            name: 'Slippers',
            size: '6',
            color: 'Blue',
            articleNo: '8475112',
            unit: '1',
            price: '$6'
        },
    ];
}
